...
from tvsorter.filesystem import expand_source_files, expand_video_files, resolve_under_root
...
def test_expand_source_files_includes_non_video_files(tmp_path: Path) -> None:
    root = tmp_path / "root"
    root.mkdir()
    (root / "installer.7z").write_text("archive")
    (root / "episode.mkv").write_text("video")

    files = expand_source_files(root, [""])

    assert files == [(root / "episode.mkv").resolve(), (root / "installer.7z").resolve()]
